...
const FORMAT_OPTIONS: {
  ...
  { value: "png", label: "PNG", description: "Lossless, widest support" },
...
export default function ImageBackgroundRemover() {
  ...
  async function removeBackground() {
    ...
    try {
      ...
      setStatusText("Creating background removal job...");
      ...
      if (!job.data?.jobId) throw new Error("The background removal job could not be created.");
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        ...
        if (currentJob.status === "failed") {
          throw new Error(currentJob.error?.message || "Background removal failed.");
        }
      ...
      if (!completedJob.outputFileId) {
        throw new Error("Processing completed but no output file was returned.");
      }
      ...
      if (!downloadResponse.ok) {
        throw new Error(`Unable to retrieve the processed image (${downloadResponse.status}).`);
      }
      ...
      const outputMimeType = outputBlob.type || (format === "webp" ? "image/webp" : "image/png");
      ...
      console.error("Background removal failed:", removalError);
      setProgress(0);
      setStatusText("");
      setError(
        ...
        <div className="space-y-5 sm:space-y-6">
          <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
            ...
            >
              <div className="flex min-h-[280px] items-center justify-center p-4 sm:min-h-[420px]">
                ...
                <img
                  ...
                  alt={result ? "Background removed" : `Preview of ${file.name}`}
            ...
            <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,0.06)] sm:p-6">
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-blue-600">Background removal</p>
              <h3 className="mt-2 text-xl font-black tracking-[-0.02em] text-slate-950">Choose output format</h3>
              <p className="mt-2 text-sm leading-6 text-slate-500">Both formats keep transparency where the background was removed.</p>
          ...
          {result && (
            <div className="rounded-[28px] border border-emerald-200 bg-emerald-50/60 p-5 shadow-sm sm:p-6">
              <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-700">Background removed</p>
