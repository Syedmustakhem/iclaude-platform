...
async function convertHeicToJpg(
  ...
  /*
   * IMPORTANT:
   * heic2any accesses browser APIs such as `window`.
   *
   * Do NOT import it at module level because Next.js
   * prerenders this page on the server.
   *
   * Loading it dynamically here ensures it is only loaded
   * inside the browser when the user actually converts a file.
   */
...
export default function HeicToJpg() {
  ...
  /*
   * Clean up generated object URLs whenever the component
   * unmounts. We intentionally do this through a ref so
   * cleanup always sees the latest items.
   */
      ...
      {items.length === 0 && (
        ...
        >
          ...
          <p className="mt-2 text-sm text-slate-500">
            or click to choose HEIC or HEIF images
          </p>
        ...
        <>
          ...
          <div className="space-y-3">
            {items.map((item) => (
              ...
              >
                <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-xl border border-slate-100 bg-slate-50 text-xl">
                  {item.status === "done" ? "✓" : "🖼️"}
                </div>
