  ...
  return new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      try {
        ...
        // Paint a white background before drawing the WebP.
    ...
    image.onerror = () => {
      ...
      reject(
        new Error(
          "The WebP image could not be loaded.",
        ),
      );
    };
...
export default function WebpToJpg() {
  ...
  const addFiles = useCallback((files: File[]) => {
    ...
    const webpFiles = files.filter(isWebp);

    const invalidTypeCount =
      files.length - webpFiles.length;
    ...
    if (invalidTypeCount > 0) {
      errors.push("Only WebP files are supported.");
    }
  ...
  const convertAll = async () => {
    ...
    try {
      for (const item of items) {
        ...
        try {
          const blob = await convertWebpToJpg(
            item.file,
            quality,
          );
  ...
  return (
    <div className="space-y-6">
      {items.length === 0 && (
        ...
        >
          ...
          <p className="mt-2 text-sm text-slate-500">
            or click to choose WebP images
          </p>
      ...
      <p className="text-center text-xs leading-6 text-slate-400">
        WebP images are converted directly in your browser.
